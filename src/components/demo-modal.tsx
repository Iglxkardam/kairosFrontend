...
export function DemoModal({ open, mode, onClose }: { open: boolean; mode: Mode; onClose: () => void }) {
  useEffect(() => {
    if (!open) return;
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 grid place-items-center p-4">
      <div className="fade-in absolute inset-0 bg-black/50 backdrop-blur-md" />
      ...
      <div role="dialog" aria-modal="true" className="pop-in relative w-full max-w-md rounded-3xl border border-line bg-card p-6 shadow-2xl">
        ...
        <h2 className="mt-4 font-display text-xl font-extrabold">
          {mode === "signup" ? "No sign-up needed" : "No login needed"}
        </h2>
